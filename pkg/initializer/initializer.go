...
func resolveOptions(opts *InitOptions, invoker projectriff_v1.Invoker) error {
	...
	if opts.Handler != "" {
		handler, err := ApplyTemplate(opts.Handler, "opts.Handler", handlerOptions{FunctionName: opts.FunctionName})
		if err != nil {
			return err
		}
